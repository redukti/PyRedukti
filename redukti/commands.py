...
def read_curvedefinitions(filename):
    interpolator_types = {
        'Linear': enums.LINEAR,
...
def read_parcurves(filename):
    ...
    return par_curve_set


def do_build_curves(stub, curve_definitions_filename, par_rates_filename):
    definitions = read_curvedefinitions(curve_definitions_filename)
    # print(definitions)
    par_curve_set = read_parcurves(par_rates_filename)
    # print(par_curve_set)
    request = services_pb2.Request()
    # FIXME date must be a parameter
    request.bootstrap_curves_request.business_date = redukti.Date.from_dmy(11,12,2012).serial()
    request.bootstrap_curves_request.curve_definitions.extend(definitions)
    request.bootstrap_curves_request.par_curve_set.CopyFrom(par_curve_set)
    response = stub.serve(request)
    if response.header.response_code != 0:
        raise Exception(response.header.response_message)
...
def run(args):
    with grpc.insecure_channel('localhost:9000') as channel:
        ...
        if args.command == 'hello':
            ...
            do_build_curves(stub, args.input1, args.input2)
        else:
